...
/**
 * Sends webhook HTTP POST calls asynchronously and persists the response status. Each call runs in
 * its own Spring-managed async thread with transactional context.
 */
@Component
public class WebhookSender {
  ...
  public WebhookSender(
    ...
    this.restClient =
        Objects.requireNonNull(webhookRestClient, "webhookRestClient must not be null");
  ...
  protected void sendAndTrack(final WebhookDto webhook, final WebhookEventPayload payload) {
    ...
    try {
      ...
      LOG.warn(
          "Webhook call returned client error for URL {}: {}", webhook.url(), e.getStatusCode());
    } catch (final HttpServerErrorException e) {
      status = e.getStatusCode().value();
      LOG.warn(
          "Webhook call returned server error for URL {}: {}", webhook.url(), e.getStatusCode());
    ...
    final WebhookDto updated =
        new WebhookDto(webhook.id(), webhook.url(), webhook.active(), status, Instant.now());
  ...
  public void ping(final UUID id) {
    ...
    final WebhookDto dto =
        webhookDataService
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Webhook with id " + id + " not found"));
