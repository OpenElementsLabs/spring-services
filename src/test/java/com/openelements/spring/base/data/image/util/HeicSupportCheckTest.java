...
class HeicSupportCheckTest {
  ...
  void probeResourceIsBundled() {
    try (InputStream sample =
        HeicSupportCheck.class.getResourceAsStream("/heic-probe/sample.heic")) {
  ...
  void realEnvironmentReturnsFalseWithoutHeicLibrary() {
    ...
    final boolean heicReaderPresent =
        java.util.Arrays.stream(ImageIO.getReaderFormatNames())
